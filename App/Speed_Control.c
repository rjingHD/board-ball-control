...
void speed_decision(void)
{
    Speed_Read();
  //  SetMotorSpeed();
    Diff_Speed(serve_value);//参数为舵机转角量
    SpeedControl();
}
void SetMotorSpeed(void)
{
    ...
    if(mode1==1)
    {
        if(effctline<4 && effctline-lasteffctline>0) MotorSpeed = 108;
        if(effctline<8 && effctline>=4 && effctline-lasteffctline>0) MotorSpeed = 109;
        if(effctline<13 && effctline>=8 && effctline-lasteffctline>0) MotorSpeed =108;
        if(effctline<18 && effctline>=13 && effctline-lasteffctline>0) MotorSpeed = 107;
        if(effctline>=18 && deltax*Control_error>0) MotorSpeed = 100;
        if(effctline>30) MotorSpeed = 93;
        if(obstacle_flag) MotorSpeed = 93;
        // MotorSpeed = 80;
        if(ring_flag)
        {
          MotorSpeed = 86;
     ...
     if(mode2==1)
    {
        if(effctline<4 && effctline-lasteffctline>0) MotorSpeed = 95;
        if(effctline<8 && effctline>=4 && effctline-lasteffctline>0) MotorSpeed = 94;
        if(effctline<13 && effctline>=8 && effctline-lasteffctline>0) MotorSpeed = 93;
        if(effctline<18 && effctline>=13 && effctline-lasteffctline>0) MotorSpeed = 92;
        if(effctline>=18 && deltax*Control_error>0) MotorSpeed = 88;
        if(effctline>30) MotorSpeed = 70;
        if(obstacle_flag) MotorSpeed = 80;
        //MotorSpeed = 80;
        if(ring_flag)
        {
          MotorSpeed = 80;
...
//    if(runtime>400)
//    {
//        if((stop&&stopcar()&& stop_line>=50)||stop_of_outroad)
//        {
